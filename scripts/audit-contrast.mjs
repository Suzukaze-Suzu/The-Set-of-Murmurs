...
// ↓ 第2j步：实底上的字改成**该分类自己的同色系墨色**（不再用黑字）——用户原话
//   「你怎么全改成黑色字体了，我让你把字改为同颜色的深色版本」。数值如实打印、标记为已知取舍。
A('var(--sky-ink)', '#5BA8D8', '同色系墨字 on 天空蓝实底（读后感小签）', 4.5, '2.14:1；用户第2j步点名要「同颜色的深色版本」（黑字那版是 4.85:1）');
A('var(--coral-ink)', '#E89B8A', '同色系墨字 on 珊瑚粉实底（随笔小签）', 4.5, '2.61:1；同上（黑字那版是 5.73:1）');
A('var(--honey-ink)', '#E8C9A0', '同色系墨字 on 蜜金实底（小说小签）', 4.5, '3.38:1；同上（黑字那版是 8.03:1）');
