...
// ↓ 第2k步（2026-09-14）：分类小签的字**一律白字**——用户原话
//   「在每一个文章左上角小圆圈内的分类文字用白色」。
//   第2j步那版「同色系墨字压同色实底」实测只有 2.14 / 2.61 / 3.38:1（糊），本步五类全改白字。
//   数值如实打印、标记为已知取舍（蜜金 1.58:1 最低，用户点名要白字）。
A('#ffffff', '#5BA8D8', '白字 on 天空蓝实底（读后感小签）', 4.5, '2.61:1；用户第2k步点名「分类文字用白色」（第2j步的同色系墨字那版只有 2.14:1）');
A('#ffffff', '#E89B8A', '白字 on 珊瑚粉实底（随笔小签）', 4.5, '2.21:1；同上（第2j步墨字那版 2.61:1）');
A('#ffffff', '#E8C9A0', '白字 on 蜜金实底（小说小签）', 4.5, '1.58:1；同上——五类白字里最低的一档（第2j步墨字那版 3.38:1），要拉回 4.5:1 只能把实底换成蜜金 12% 浅档');
A('#ffffff', '#4A9BB8', '白字 on 青蓝实底（数学笔记小签）', 3);
A('#ffffff', '#8A8F9A', '白字 on 灰蓝实底（学习分享小签）', 3);
...
// ↓ 第2f步：--sky-fill（墨蓝实底 #2E6E92）亮色已弃用 → 实底改用色卡天空蓝本身；
//   第2j步：这些实底上的字改成同色系墨色（--sky-ink），不再用黑字。
A('var(--sky-ink)', 'var(--sky-blue)', '同色系墨字 on 天空蓝实底（筛选/留言板 tab/回复/bug/阅读器/友链 hover）', 4.5, '2.14:1；用户第2j步点名要「同颜色的深色版本」（黑字那版是 4.85:1）');
...
// ↓ 第2l步：暗色下分类小签的字＝**该分类的色卡色本身**（＝「亮色版」那一档）——用户原话
//   「让暗色下的这个区域的文字全部变成亮色的凉风凉四色」。底仍是第2g步那条「13% 该分类色淡底」
//   （= color-mix(色卡色 13%, transparent) 压在该分类卡片 #1e2937 上，脚本按 mix() 实算）。
//   改前走 --cat-ink（同色系墨色，其中三类还是写死的深色值）只有 1.86~2.10:1，几乎看不见。
D('#5BA8D8', mix('#5BA8D8', dark['--bg-card'], 0.13), '暗色小签 天空蓝字 on 天空蓝13%底', 4.5, '4.49:1（差 0.01）；改前 --cat-ink 只有 2.10:1，本步已是大幅改善');
D('#E89B8A', mix('#E89B8A', dark['--bg-card'], 0.13), '暗色小签 珊瑚粉字 on 珊瑚粉13%底');
D('#E8C9A0', mix('#E8C9A0', dark['--bg-card'], 0.13), '暗色小签 蜜金字 on 蜜金13%底');
D('#4A9BB8', mix('#4A9BB8', dark['--bg-card'], 0.13), '暗色小签 青蓝字 on 青蓝13%底', 4.5, '3.85:1；青蓝本身是中调色，压 13% 淡底到不了 4.5（改前 1.86:1）');
D('#8A8F9A', mix('#8A8F9A', dark['--bg-card'], 0.13), '暗色小签 灰蓝字 on 灰蓝13%底', 4.5, '3.77:1；灰蓝本身是中调色（改前 2.03:1）');
