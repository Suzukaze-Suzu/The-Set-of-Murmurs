...
## 原理
纯前端（浏览器）拿不到邮箱、也发不了邮件，所以用 Netlify 的**服务端函数（Netlify Functions）**代发：
...
## 在 Netlify 配置环境变量
Netlify → 你的站点 → Site settings → Environment variables → 添加：
...
## 部署验证
1. 代码推送到 GitHub，Netlify 自动部署（Functions 会自动打包）
2. 用 A 账号在文章下评论，再用 B 账号回复 A 的评论
3. A 的邮箱收到「新回复通知」邮件
4. 本地测试可运行 `netlify dev`（需要安装 netlify-cli），或用 `npm i -g netlify-cli`

## 常见问题
- 邮件没收到：检查 Netlify Functions 日志（Functions → comment-notify → Logs）看报错；确认 RESEND_API_KEY 正确
